...
import static org.apache.hadoop.fs.cosn.commit.InternalCommitterConstants.*;
import static org.apache.hadoop.fs.cosn.commit.ValidationFailure.verify;
...
public final class CommitUtils {
   ...
   */
  public static void verifyIsMagicCommitPath(CosNFileSystem fs,
   ...
   */
  public static void verifyIsMagicCommitFS(CosNFileSystem fs)
   ...
   */
  public static CosNFileSystem verifyIsCosFS(FileSystem fs, Path path)
      throws PathCommitException {
    if (!(fs instanceof CosNFileSystem)) {
      throw new PathCommitException(path, E_WRONG_FS);
    }
    return (CosNFileSystem) fs;
  }
   ...
   */
  public static CosNFileSystem getCosFileSystem(Path path,
      Configuration conf,
      boolean magicCommitRequired)
      throws PathCommitException, IOException {
    CosNFileSystem cosFS = verifyIsCosFS(path.getFileSystem(conf), path);
    if (magicCommitRequired) {
      verifyIsMagicCommitFS(cosFS);
    }
    return cosFS;
  }
