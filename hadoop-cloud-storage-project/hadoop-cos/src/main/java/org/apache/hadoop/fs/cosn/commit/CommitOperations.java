...
public class CommitOperations {
  ...
  /** Statistics. */
  private final org.apache.hadoop.fs.cosn.COSInstrumentation.CommitterStatistics statistics;
  ...
  private final NativeFileSystemStore writeOperations;
   ...
   */
  public CommitOperations(CosNFileSystem fs) {
    Preconditions.checkArgument(fs != null, "null fs");
    this.fs = fs;
    // todo: how to add this to cos
  ...
  /** @return statistics. */
  protected org.apache.hadoop.fs.cosn.COSInstrumentation.CommitterStatistics getStatistics() {
    return statistics;
  }
  ...
  private long innerCommit(
    ...
    // finalize the commit
    // todo tmd
    writeOperations.completeMultipartUpload(
  ...
  private void abortMultipartCommit(String destKey, String uploadId)
      throws IOException {
    try {
      writeOperations.abortMultipartUpload(destKey, uploadId);
    } finally {
   ...
   * this method only used for test in s3a committer
    ...
    while (pendingFiles.hasNext()) {
      ...
      if (pendingFile.getName().endsWith(CommitConstants.PENDING_SUFFIX)) {
        try {
          ...
          CosNUtils.deleteQuietly(fs, pendingFile, false);
        }
      }
    }
    return outcome;
  ...
  public void createSuccessMarker(Path outputPath,
    ...
    successData.addDiagnostic(COS_METADATA_STORE_IMPL,
        conf.getTrimmed(COS_METADATA_STORE_IMPL, ""));
  ...
  public void revertCommit(SinglePendingCommit commit) throws IOException {
    ...
    try {
      fs.revertCommit(commit.getDestinationKey());
    } finally {
  ...
  public SinglePendingCommit uploadFileToPendingCommit(File localFile,
    ...
    try {
      ...
      for (int partNumber = 1; partNumber <= numParts; partNumber += 1) {
        ...
        // todo get the upload part with single request
  ...
  public CommitContext initiateCommitOperation(Path path) throws IOException {
    // todo watch out this commit context remove the operation guard context param
   ...
   * visible. It contains a {@link } field, which, if
  ...
  public final class CommitContext implements Closeable {
     ...
     */
    // todo this part to drop the bulk operation to avoid involve the s3a guard
