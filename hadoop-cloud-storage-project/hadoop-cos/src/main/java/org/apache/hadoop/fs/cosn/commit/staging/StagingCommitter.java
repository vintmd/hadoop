...
import org.apache.hadoop.fs.cosn.commit.AbstractCOSCommitter;
import org.apache.hadoop.fs.cosn.commit.CommitConstants;
import org.apache.hadoop.fs.cosn.commit.CommitOperations;
import org.apache.hadoop.fs.cosn.commit.InternalCommitterConstants;
import org.apache.hadoop.fs.cosn.commit.Tasks;
import org.apache.hadoop.fs.cosn.commit.files.PendingSet;
import org.apache.hadoop.fs.cosn.commit.files.SinglePendingCommit;
...
import static org.apache.hadoop.fs.cosn.CosNUtils.*;
import static org.apache.hadoop.fs.cosn.commit.staging.StagingCommitterConstants.*;
import static org.apache.hadoop.fs.cosn.commit.CommitConstants.*;
import static org.apache.hadoop.fs.cosn.commit.CommitUtils.*;
import static org.apache.hadoop.fs.cosn.commit.CommitUtilsWithMR.*;
 ...
 */
public class StagingCommitter extends AbstractCOSCommitter {
  ...
  public StagingCommitter(Path outputPath,
    ...
    CosNFileSystem fs = getCosFileSystem(finalOutputPath,
   ...
   * <ol>
   *   <li>Value of {@link InternalCommitterConstants#FS_COS_COMMITTER_STAGING_UUID}.</li>
  ...
  public static String getUploadUUID(Configuration conf, String jobId) {
    return conf.getTrimmed(
        InternalCommitterConstants.FS_COS_COMMITTER_STAGING_UUID,
  ...
  protected final Path getFinalPath(String relative, JobContext context)
      throws IOException {
    return getDestCosFS().keyToQualifiedPath(getFinalKey(relative, context));
  }
  ...
  public void setupJob(JobContext context) throws IOException {
    ...
    context.getConfiguration().set(
        InternalCommitterConstants.FS_COS_COMMITTER_STAGING_UUID, uuid);
  ...
  public void cleanupStagingDirs() {
    ...
    if (workPath != null) {
      ...
      try {
        deleteQuietly(workPath.getFileSystem(getConf()), workPath, true);
      } catch (IOException e) {
        LOG.debug("clean up staging dirs failed {}", workPath.toString(), e);
      }
    }
  }
  ...
  protected void deleteDestinationPaths(JobContext context) throws IOException {
    ...
    LOG.info("Deleting Job attempt Path, {}", attemptPath.toString());
    try {
      deleteWithWarning(getJobAttemptFileSystem(context), attemptPath, true);
    } catch (IOException e) {
      LOG.debug("Delete destination paths failed {}", attemptPath.toString(), e);
    }
      ...
      try {
        Tasks.foreach(taskOutput)
            ...
            .run(stat -> {
              ...
              Path destPath = getDestCosFS().keyToQualifiedPath(key);
  ...
  protected void deleteTaskWorkingPathQuietly(JobContext context) {
    ...
    try {
      ...
      LOG.debug("delete task working path quiet", e);
    }
  }
